...
export const CategoryPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<TaskStatus | 'Projects'>('Today');
  ...
  const [newTask, setNewTask] = useState<{ title: string; xp: number; duration: number; status: TaskStatus; projectId?: string }>({
  ...
  const filteredTasks = tasks.filter(t => t.category === decodedCategory && t.status === activeTab);
    ...
    <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
             ...
             <Card className="min-h-[500px] flex flex-col border-slate-200 shadow-sm">
                <div className="p-4 border-b border-slate-100 flex flex-wrap gap-2 items-center justify-between bg-slate-50/50 rounded-t-lg">
                    <div className="flex gap-1 bg-slate-100 p-1 rounded-lg overflow-x-auto">
                        {(['Today', 'This Week', 'Backlog', 'Projects', 'Done'] as const).map(tab => (
                            <button
                                key={tab}
      ...
      <Modal isOpen={showTaskModal} onClose={() => setShowTaskModal(false)} title="Create XP Task">
            <form onSubmit={handleAddTask} className="space-y-4">
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">When to do?</label>
                    <Select
